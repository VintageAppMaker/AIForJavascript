// Chrome에서 F12 후, 녹음기 기능으로 macro 저장함
// puppeteer의 소스를 내려받을 수 있음.
// 업로드도 가능함. 
// 아직은 실험실 아이콘으로 제공함
...
(async () => {
    ...
    }
    {
        ...
        if (inputType === 'select-one') {
          await changeSelectElement(element, 'ww')
        } else if ([
            'textarea',
            'text',
            'url',
          ...
          await typeIntoElement(element, 'ww');
        } else {
          await changeElementValue(element, 'ww');
        }
    }
    {
        const targetPage = page;
    ...
    }
    {
        ...
        const inputType = await element.evaluate(el => el.type);
        if (inputType === 'select-one') {
          await changeSelectElement(element, 'www.na')
        } else if ([
            'textarea',
            'text',
            'url',
          ...
          await typeIntoElement(element, 'www.na');
        } else {
          await changeElementValue(element, 'www.na');
        }
    }
    {
        const targetPage = page;
    ...
    }
    {
        ...
        const inputType = await element.evaluate(el => el.type);
        if (inputType === 'select-one') {
          await changeSelectElement(element, 'naver')
        } else if ([
            'textarea',
            'text',
            'url',
          ...
          await typeIntoElement(element, 'naver');
        } else {
          await changeElementValue(element, 'naver');
        }
    }
    {
        const targetPage = page;
